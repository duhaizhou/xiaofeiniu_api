...
router.get('/login/:aname/:apwd',(req,res)=>{
 ...
 /**
 * API:PATCH/admin       //修改部分信息用patch，全部信息用put
 * 请求数据：{aname:'xxx',oldPwd:'xxx',newPwd:'xxx'}
 * 根据管理员用户名和密码修改管理员密码
 * 返回数据：
 * {code:200,msg:'modified succ'}
 * {code:400,msg:'aname or apwd err'}
 * {code:401,msg:'awpd not modified'}
 */
router.patch('/',(req,res)=>{
  var data=req.body;
  //首先根据aname/oldPwd查询该用户是否存在
  pool.query('SELECT aid FROM xfn_admin WHERE aname=? AND apwd=PASSWORD(?)',[data.aname,data.oldPwd],
  (err,result)=>{
    if(err) throw err;
    if(result.length>0){ //如果查询到了用户，再修改其密码 
      
      pool.query('UPDATE xfn_admin SET apwd=PASSWORD(?) WHERE aname=?',[data.newPwd,data.aname],
      (err,result)=>{
        if(err) throw err;
        ...
        if(result.changedRows>0){//密码修改成功
          ...
          res.send({code:401,msg:'awpd not modified'})
        }
      })
    }else{
      res.send({code:400,msg:'aname or apwd err'})
